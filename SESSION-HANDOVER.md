...
| tier | red mutants |
|---|---|
| model (default suite) | `rhombus_half_pixel_cap` and `rhombus_half_pixel_headroom` — filed by a curator on 2026-08-19, and the narrowest reds this row has ever held: canvas.py computes the client's own bound-text cap and headroom with Python's round, which breaks halves to EVEN, where Math.round in the shipped bundle breaks them UP. The two part only where the halved extent is a tie AND its floor is even — w = 4k+1 and nothing else, 495 of the 1981 integer widths in 20..2000, none of them the corpus's one distinct diamond — so on a 181px rhombus the client allows 81px and the check calls an 81px label too wide for 80. The height twin went unseen longer because text_dims quantizes to an even number at fontSize 16 whatever the text does, so nothing could land in the one-pixel gap; its scene uses 20, where it can. One pixel is under every tolerance band in the file, so both scenes are built on the boundary rather than lifted from a fixture. Owned by whoever owns the transcribed Math.round sites — there are five, not the two first reported — and flipped by floor(x + 0.5) at all of them; the catalogue entry records why ceil also flips both and is still wrong, and the fractional case that catches it. Any id listed here is a live red owned by its own fix task. This row states what is red, never how many drains it has seen — the count is derived below and the narrative is not, so a claim about the row's *history* is one nothing checks. (Backtick an id here ONLY as a live red: the guard below harvests every backticked token in the row, so a backticked function name reads as a live red id) |
...
- `python3 tests/test_mutants.py --coverage` — one row per detector: proven
  ...
  <!-- live:detector_coverage_totals -->**33 detectors, 30 proven, 3 render-tier, 0 UNCOVERED**<!-- /live:detector_coverage_totals -->
