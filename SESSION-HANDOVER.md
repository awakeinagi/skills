...
| tier | red mutants |
|---|---|
| model (default suite) | `rhombus_half_pixel_cap` — filed by a curator on 2026-08-19, and the narrowest red this row has held: canvas.py computes the client's own bound-text cap with Python's round, which breaks halves to EVEN, where Math.round in the shipped bundle breaks them UP. The two agree at every width except the ones where w / 2 lands on a half, so on a 181px rhombus the client allows 81px and the check calls an 81px label too wide for 80. One pixel; no tolerance band in the file can see it, so the scene is placed on the boundary instead. Owned by whoever owns the two halving sites (the wrap width and the text headroom) and flipped by floor(x + 0.5); the catalogue entry records why ceil also flips it and is still wrong. It is the residue of an investigation that cleared the padding constant, the three shape arms and the advance table in turn. The chopped-token red before it (spelled out rather than backticked, for the reason at the end of this row) was filed the same day and drained the same day by v0.9 WP4-AND-GUARDS one commit later, for a bound label called merely too wide where the browser also drew it too TALL. Any id listed here is a live red owned by its own fix task. This row states what is red, never how many drains it has seen — the count is derived below and the narrative is not, so a claim about the row's *history* is one nothing checks. (Backtick an id here ONLY as a live red: the guard below harvests every backticked id in the row, so an id mentioned as history would read as a live red) |
