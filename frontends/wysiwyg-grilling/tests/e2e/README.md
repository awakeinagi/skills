...
- **unittest** — routing, commit/replay, lints, registry, facts,
  ...
  <!-- live:unittest_suite_cases -->1786<!-- /live:unittest_suite_cases -->
...
| Does the app AUTO-GROW a container instead of spilling its label? | e2e-flow (planned, run 6) | **filed 2026-08-18 by v0.9 TASK-ENTITY-LINEHEIGHT; nothing tests it today.** Load a scene whose bound label is taller than its container at the label's own `lineHeight` — the model tier's `_line_height_box(height=64, line_height=1.35)` is the exact case, 3 lines of 16px in a 64px box — and read back what the app stores: does the container grow, or does the ink hang 4px below it? NOT DECIDABLE ANYWHERE ELSE, including the render tier, which rasterizes `canvas.render_svg`'s markup and never loads the app. What it settles is what the OVERFLOW WARNING should say — "widen the box" versus "the app will widen it for you" — not whether the two readers of `lineHeight` should agree; that half is closed (`TestOneLineHeightHasTwoReaders`, both poles green). Sits beside `placement.spec` because it is the same question shape: what does the real client do with geometry we chose |
